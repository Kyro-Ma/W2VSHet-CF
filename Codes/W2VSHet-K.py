...
def train_and_evaluate(training_data, testing_data, items_dict):
    ...
    print_counter = 20000
    uids_and_iids_of_knn_prediction = [(p.uid, p.iid) for p in test_pred]
    ...
    for index, row in training_data.iterrows():
        ...
        if (row['user_id'], row['parent_asin']) in uids_and_iids_of_knn_prediction:
            if row['user_id'] in uid_train.keys():
                pass
            else:
                uid_train[row['user_id']] = current_uid_train
    ...
    # iid_train_keys = iid_train.keys()
    # iid_test_keys = iid_test.keys()
    ...
    review_train = []
    knn_rating_edge_from_train, knn_rating_edge_to_train = [], []
    knn_rating_train = []
    counter = 0
    ...
    for _, row in training_data.iterrows():
        ...
        if (row['user_id'], row['parent_asin']) in uids_and_iids_of_knn_prediction:
            knn_rating_edge_from_train.append(uid_train[row['user_id']])
            knn_rating_edge_to_train.append(iid_train[row['parent_asin']])

            pos = uids_and_iids_of_knn_prediction.index((row['user_id'], row['parent_asin']))
            knn_rating_train.append(test_pred[pos].est)
        ...
        if row['text'] is not None:
            ...
            review_train.append(get_word2vec_sentence_vector(row['title'], w2v_model, vector_size))
    ...
    data_train['user', 'KNN_rates', 'item'].edge_index = torch.tensor(
        [knn_rating_edge_from_train, knn_rating_edge_to_train], dtype=torch.long
    ).to(device)
    data_train['user', 'KNN_rates', 'item'].edge_attr = torch.tensor(knn_rating_train, dtype=torch.float).to(device)
    data_train['item', 'KNN_rated_by', 'user'].edge_index = torch.tensor(
        [knn_rating_edge_to_train, knn_rating_edge_from_train], dtype=torch.long
    ).to(device)
    knn_rating_train.reverse()
    data_train['item', 'KNN_rated_by', 'user'].edge_attr = torch.tensor(knn_rating_train, dtype=torch.float).to(device)
    ...
    review_test = []
    knn_rating_edge_from_test, knn_rating_edge_to_test = [], []
    knn_rating_test = []
    counter = 0
    ...
    for _, row in testing_data.iterrows():
        ...
        if (row['user_id'], row['parent_asin']) in uids_and_iids_of_knn_prediction:
            knn_rating_edge_from_test.append(uid_test[row['user_id']])
            knn_rating_edge_to_test.append(iid_test[row['parent_asin']])

            pos = uids_and_iids_of_knn_prediction.index((row['user_id'], row['parent_asin']))
            knn_rating_test.append(test_pred[pos].est)
        ...
        if row['text'] is not None:
            ...
            review_test.append(get_word2vec_sentence_vector(row['title'], w2v_model, vector_size))
    ...
    data_test['user', 'KNN_rates', 'item'].edge_index = torch.tensor(
        [knn_rating_edge_from_test, knn_rating_edge_to_test], dtype=torch.long
    ).to(device)
    data_test['user', 'KNN_rates', 'item'].edge_attr = torch.tensor(knn_rating_test, dtype=torch.float).to(device)
    data_test['item', 'KNN_rated_by', 'user'].edge_index = torch.tensor(
        [knn_rating_edge_to_test, knn_rating_edge_from_test], dtype=torch.long
    ).to(device)
    knn_rating_test.reverse()
    data_test['item', 'KNN_rated_by', 'user'].edge_attr = torch.tensor(knn_rating_test, dtype=torch.float).to(device)
    ...
    data_test['item', 'same_store', 'item'].edge_index = torch.tensor(
        [same_store_edge_test[0] + same_store_edge_test[1],
         same_store_edge_test[1] + same_store_edge_test[0]]
    ).to(device)
    ...
    class HeteroGNN(torch.nn.Module):
        def __init__(self, num_users, hidden_channels, item_features_dim):
            ...
            self.conv1 = HeteroConv({
                ...
                ('user', 'KNN_rates', 'item'): SAGEConv((item_features_dim, item_features_dim), hidden_channels),
                ('item', 'KNN_rated_by', 'user'): SAGEConv((item_features_dim, item_features_dim), hidden_channels),
...
if __name__ == '__main__':
    ...
    hidden_channels = 16
    threshold_for_fashion = [4, 5, 6, 7]
    threshold_for_beauty = [4, 5, 6, 7]
    threshold_for_music = [4, 5, 6, 7]
    threshold_for_movie = [4, 5, 6, 7]
    ...
    for df_path, meta_df_path, w2vec_path, threshold in zip(df_path_list, meta_df_path_list, w2vec_path_list, threshold_list):
        ...
        for local_threshold in threshold:
            ...
            if identifier == "Musical_Instruments" or identifier == "Movies_and_TV":
                df = df[0: 500000]
            ...
            # region create a new_df for KNN
            selected_columns = ['rating', 'parent_asin', 'user_id']
            # Create a new DataFrame with only the selected columns
            df = df[selected_columns]

            knn_df = df.groupby("parent_asin").filter(lambda x: x['rating'].count() >= 20)
            # endregion

            # region Create a Reader object with the rating scale
            reader = Reader(rating_scale=(1, 5))
            # Load the dataset from the DataFrame
            knn_data = Dataset.load_from_df(knn_df[['user_id', 'parent_asin', 'rating']], reader)

            trainset, testset = train_test_split(knn_data, test_size=0.3, random_state=10)

            sim_options = {'name': 'pearson_baseline', 'user_based': False}  # item-item
            algo4 = KNNBaseline(k=5, sim_options=sim_options)
            algo4.fit(trainset)
            test_pred = algo4.test(testset)
            # endregion

            # region clear unnecessary things
            del df, meta_df, item_with_empty_title, selected_columns, removed_parent_asin, reader, knn_data, knn_df
            del nodeSGModel, sequence, sequence_shuffled
            gc.collect()
            # endregion
        ...
        if 'beauty' in df_path:
            output_path = f'../Datasets/Word2vec+SkipGram+KNN+HeteroGNN_hidden_channels={hidden_channels}_layers={layers}_{identifier}.xlsx'
        else:
            output_path = f'../Datasets/Word2vec+SkipGram+KNN+HeteroGNN_hidden_channels={hidden_channels}_layers={layers}_{identifier}.xlsx'
    ...
    '''
    word2vec + SkipGram + KNN + HeteroGNN on beauty
    asin_rating_num = 30 (RMSE:2.00, MAE: 1.18)
    [2.0225947749307127, 2.0108450153370163, 1.9802700032630616, 1.991768161696906, 2.0173724520266605]
    [1.1896434859437532, 1.1873792239631664, 1.1721513076974897, 1.1809306792236118, 1.191989208259341]
    
    asin_rating_num = 20 (RMSE:2.00, MAE: 1.18)
    [2.0237113592422973, 2.0108858065489366, 1.9819110824367607, 1.9923292843166611, 2.015888052606647]
    [1.1908239188256198, 1.1877487938152564, 1.1732699728931948, 1.1812446754661023, 1.1906925072537915]
    '''

    '''
    word2vec on fashion
    asin_rating_num = 30 (RMSE:1.80, MAE: 1.14)
    [1.8003772150904833, 1.8000094708703533, 1.7914012164452549, 1.7860774379926252, 1.812659648330336]
    [1.135361186665185, 1.1368464985507558, 1.1363689873957705, 1.1304736876157793, 1.1441744878173528]
    asin_rating_num = 20 (RMSE:1.80, MAE: 1.14)
    [1.7973907457438218, 1.7969216691673653, 1.8007268969625436, 1.7870634520985564, 1.816556137031519]
    [1.1349321377662356, 1.136098478390315, 1.1394885891984434, 1.1316485097979798, 1.1456258453013792]
    '''